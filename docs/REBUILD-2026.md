# sve-ui — 2026 Rebuild (COMPLETE — historical record)

> **This project is finished.** It is kept for its decision log, its architecture
> notes and its compatibility research, all of which are still the reference for
> how the library is built. It is no longer a plan.
>
> Shipped as `sve-ui@0.7.0`. All 59 registry entries live, 561 tests, automated
> axe and SSR suites, tree-shaking and `.d.ts` guards in CI, Trusted Publishing.
>
> Forward-looking work lives in [`../ROADMAP.md`](../ROADMAP.md).
>
> **Four items below are still marked `- [ ]` but were in fact delivered.** They
> are left as-written rather than back-edited, because the gap between a
> checkbox and reality is the most useful thing this document now records:
>
> | Line                       | Says        | Actually                                                                                         |
> | -------------------------- | ----------- | ------------------------------------------------------------------------------------------------ |
> | Box/Stack/Flex             | open        | `Stack` + `Flex` shipped; `Box` deliberately dropped (it had reinvented CSS with a worse syntax) |
> | Remaining Bits primitives  | 19 left     | every one wrapped; the registry has no `soon` entries                                            |
> | Auto-generated prop tables | open        | generator shipped — but **27 of 70 docs pages still ignore it**, so this one was only half true  |
> | Migration notes for 0.1.x  | not written | `MIGRATION.md`, 9 KB                                                                             |
...
- `.svelte.d.ts` files are hand-written and committed — these are _generated_ by `svelte-package`; the committed ones have already drifted.
...
| D11 | **No** shared `types` package                                                                                                       | Library types _are_ the public API, exported from `sve-ui`; nothing to share                                                 |
...
- [x] **Sidebar (composable app-shell nav):** `Sidebar.Provider/Root/Trigger/Header/Content/Footer/Group/GroupLabel/Menu/Item` + collapsed state via context + `collapsible` of icon|offcanvas|none. Custom (Bits ships none).
  ...
  - **DECIDED: the partial port is not happening either.** The reusable overlap is `Group`/`GroupLabel`/`Menu`/`Item`, and two objections raised earlier turned out to be wrong on inspection — the parts use `ctx?.` optional chaining so they need no `Provider`, and `Item` takes a `children` snippet so the `new`/`soon` badges fit inside it. What kills it is the palette: `--doc-*` are literal hex values in `apps/docs/src/app.css` (`--doc-border: #26262f`) with no relationship to `--sve-*`. The docs site keeps a separate palette on purpose, so its chrome stays put while the library theme it showcases changes. A ported nav would take its colours from `--sve-*` and visually diverge from the header, footer and cards around it — unless overridden, which is the same fighting-the-component problem at smaller scale.
  - The dogfooding value was already banked regardless: the attempt is what produced `Provider`'s `display: contents` default and `Item disabled`. Shipping the port adds no further information about the component, which was the only reason to do it.
...
- [x] **Toast (transient notifications) — the LAST component. The registry is now complete.** `toast` (imperative) + `Toast.Viewport` (declarative, required). Custom, no new runtime dependency.
  ...
  - **axe caught a real error:** `role="region"` is not allowed on `<ol>`, which already carries an implicit `list` role. Restructured to a named `<section>` (which _is_ a region) wrapping an `<ol aria-live="polite">`.
...
- **Fallback (only if OIDC unavailable):** granular _automation_ token in `NPM_TOKEN` + `--provenance` flag.
