<script lang="ts">
	import { DateRangeField } from 'sve-ui';
	...
	const meta = componentBySlug['date-range-field'];

	const toc: TocEntry[] = [
		{ id: 'usage', label: 'Usage' },
		{ id: 'naming', label: 'You must name the Root' },
		{ id: 'shared', label: 'Shared with Date Field' },
...
<DateRangeField.Root bind:value locale={userLocale} aria-labelledby="range-label">
  ...
  <DateRangeField.Input type="end">
	...
	let value = $state({ start: undefined as CalendarDate | undefined, end: undefined as CalendarDate | undefined });
</script>
...
<DocPage group={meta.group} name={meta.name} description={meta.blurb} {toc}>
	<section id="usage" class="sec">
		...
		<p class="sec__p">
			...
			<code class="ic">aria-hidden</code> — a dash read aloud between two dates adds nothing.
		</p>
		<Preview code={usageCode} align="start">
			<DateRangeField.Root bind:value locale="en-US" aria-labelledby="demo-range-label">
				<DateRangeField.Label id="demo-range-label">Stay dates</DateRangeField.Label>
				<div class="row">
					<DateRangeField.Input type="start">
						{#snippet children({ segments })}
	...
	<section id="naming" class="sec">
		...
		<p class="warn">
			Bits gives the range Root <code class="ic">role="group"</code> with <strong>no accessible
			name</strong>. Without your help the field announces as an anonymous group.
		</p>
		<p class="sec__p">
			Verified in the rendered DOM: Bits points each <code class="ic">Input</code>'s
			<code class="ic">aria-labelledby</code> at your Label, but the Inputs carry no role — so nothing
			exposed to assistive technology ends up labelled. On the single-value
			<a href="/components/date-field">Date Field</a> the Input <em>is</em> the group and Bits
			does label it, so this gap is specific to ranges.
		</p>
		<p class="sec__p">
	...
	<section id="shared" class="sec">
		<h2 class="sec__h">Shared with Date Field</h2>
		<p class="sec__p">
			<code class="ic">Segment</code> is the <strong>same component</strong>
			<a href="/components/date-field">Date Field</a> uses — Bits re-exports the identical
			module to both namespaces, so one styled implementation serves each and the two cannot drift
			apart. Everything in that page's "why not a native input" section applies here too.
		</p>
	</section>

	<section id="props" class="sec">
		<h2 class="sec__h">Props</h2>
		<p class="sec__p"><code class="ic">DateRangeField.Root</code> — <code class="ic">value</code> and <code class="ic">placeholder</code> are bindable.</p>
		<PropsTable component="DateRangeFieldRoot" />
