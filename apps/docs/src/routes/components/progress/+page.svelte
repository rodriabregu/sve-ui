...
<DocPage group={meta.group} name={meta.name} description={meta.blurb} {toc}>
	<section id="usage" class="sec">
		...
		<p class="sec__p">
			...
			<strong>name</strong> — a bar announced as just "progressbar, 40%" tells the user nothing
			about what is progressing. Always pass <code class="ic">aria-label</code>, or
			...
			<div class="stack">
				...
				<div class="row">
					<Button
						size="sm"
						variant="outline"
						onclick={() => (uploaded = Math.max(0, uploaded - 20))}
					>
						-20
					</Button>
					<Button
						size="sm"
						variant="outline"
						onclick={() => (uploaded = Math.min(100, uploaded + 20))}
					>
						+20
					</Button>
	...
	<section id="indeterminate" class="sec">
		...
		<p class="sec__p">
			Pass <code class="ic">value={null}</code> when the duration is genuinely unknown and you get a travelling
			sliver instead of a fill. Use it honestly — reaching for indeterminate because measuring progress
			is inconvenient just hides information the user wanted.
		</p>
	...
	<section id="range" class="sec">
		...
		<p class="sec__p">
			<code class="ic">min</code> and <code class="ic">max</code> define the range, and the fill is computed
			against it — values outside are clamped rather than overflowing the track.
		</p>
	...
	<section id="vs-meter" class="sec">
		...
		<p class="sec__p">
			...
			reach the end and then stop existing. <a href="/components/meter">Meter</a> reports a static measurement
			inside a known range: disk usage, password strength, a score. A meter at 100% is not "finished",
			it is full.
		</p>
		<p class="sec__p">
