...
<DocPage group={meta.group} name={meta.name} description={meta.blurb} {toc}>
	...
	<section id="usage" class="sec">
		...
		<p class="sec__p">
			Root hands you <code class="ic">months</code> and <code class="ic">weekdays</code> through a snippet
		...
		<p class="sec__p">
			...
			<code class="ic">placeholder</code> is the month on screen — both bindable, and they are genuinely
	...
	<section id="naming" class="sec">
		...
		<p class="sec__p">
			...
			the literal word <strong>"Event"</strong> — so an unset calendar announces as "Event January 2026",
		...
		<p class="sec__p">
			...
			<code class="ic">aria-label</code> to "Previous" and "Next" and merges its own props last, so
			an
	...
	<section id="states" class="sec">
		...
		<p class="sec__p">These are different answers and they are styled differently on purpose:</p>
		<ul class="sec__p">
			<li>
				<code class="ic">isDateDisabled</code> / <code class="ic">minValue</code> /
				<code class="ic">maxValue</code> → <strong>out of range</strong>. Faded out; there is
				nothing to reason about.
			</li>
			<li>
				<code class="ic">isDateUnavailable</code> → <strong>exists but taken</strong>. A fully
				booked day. Kept readable and struck through, which reads as "that date is gone" rather than
				"that date does not exist".
			</li>
		</ul>
		<p class="sec__p">
			Collapsing both into one grey blur throws away information the user needs.
			<code class="ic">data-today</code> gets a ring rather than a fill, so today stays legible even when
	...
	<section id="locale" class="sec">
		...
		<p class="sec__p">
			<code class="ic">locale</code> changes the month names,
			<strong>which day the week starts on</strong>, and the numeral system. Hardcoding Monday-first
			or Sunday-first is a correctness bug for half your users, not a styling preference. Pass the
			user's locale.
		</p>
		<p class="sec__p">
			Add <code class="ic">MonthSelect</code> and <code class="ic">YearSelect</code> for any range wider
