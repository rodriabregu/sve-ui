...
<DocPage group={meta.group} name={meta.name} description={meta.blurb} {toc}>
	<section id="usage" class="sec">
		...
		<p class="sec__p">
			...
			<code class="ic">display: flex; flex-direction: column; gap: 1rem</code>, with the gap bound to
			the spacing scale. <code class="ic">align</code> defaults to
	...
	<section id="narrow" class="sec">
		...
		<p class="sec__p">
			...
			cannot be reused in a layout that spaces things differently — you end up overriding it, which is
			worse than never having had it. And an <code class="ic">each</code>-generated list gets even
			rhythm from one <code class="ic">gap</code> rather than a margin on every item plus a
			<code class="ic">:last-child</code> exception.
		</p>
		<p class="sec__p">
			<strong>The gap is a token key, not a length.</strong> <code class="ic">gap={4}</code>, not
			<code class="ic">gap="17px"</code>. Constraining it is the whole point: it keeps vertical rhythm
			consistent across the app instead of letting every screen invent its own spacing.
		</p>
		<p class="sec__p">
	...
	<section id="as" class="sec">
		...
		<p class="sec__p">
			...
			is reset, so <code class="ic">as="ul"</code> stays semantic without looking like a bulleted
			list.
		</p>
