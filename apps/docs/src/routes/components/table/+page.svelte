...
<DocPage group={meta.group} name={meta.name} description={meta.blurb} {toc}>
	<section id="usage" class="sec">
		...
		<p class="sec__p">
			...
			<code class="ic">scope</code> on the headers. What this adds is getting that markup right by
			default, plus the styling and the scroll container on top.
		</p>
	...
	<section id="sorting" class="sec">
		...
		<p class="warn">
			This component does not sort your data. <code class="ic">Head sortable</code> renders the button
			and sets <code class="ic">aria-sort</code>; you apply the order.
		</p>
		<p class="sec__p">
			...
			<code class="ic">'á' &lt; 'b'</code> is true in Spanish and false in a plain codepoint sort. A
			built-in sort would be wrong in all three ways and hard to override in each.
		</p>
		...
		<p class="sec__p">
			Keep <code class="ic">aria-sort</code> on <strong>one</strong> column, and only when you have
			actually applied it. Announcing a sort you did not apply is a lie assistive technology has no
			way to check.
		</p>
	</section>
	...
	<section id="no-grid" class="sec">
		...
		<p class="sec__p">
			...
			override. <code class="ic">role="grid"</code> promises a full keyboard interaction model —
			arrow keys moving a focus cursor between cells, Home, End, Ctrl+Home — and a screen reader
			switches into that mode as soon as it sees the role.
		</p>
		<p class="sec__p">
	...
	<section id="scroll" class="sec">
		...
		<p class="sec__p">
			<code class="ic">Root</code> renders the horizontal scroll container itself, because a wide
			table that scrolls the whole page instead of itself is the single most common table defect.
		</p>
		<p class="sec__p">
			Pass <code class="ic">scrollLabel</code>. A scroll container that is not focusable cannot be
			scrolled without a pointer — the columns past the right edge are simply unreachable by keyboard.
			Focusability needs a name, though, so <code class="ic">tabindex</code> is only added when you
			provide one: an unnamed focusable region is announced as nothing at all.
		</p>
		<p class="sec__p">
		...
		<p class="sec__p">
			...
			<code class="ic">--sve-table-max-height</code> (24rem by default). Without a bounded height
			there is nothing for the header to stick inside and the page scrolls instead.
		</p>
	</section>
	...
	<section id="selection" class="sec">
		...
		<p class="sec__p">
			Selection also has to be <em>operable</em>, so put a real <code class="ic">Checkbox</code> in the
			first cell and let <code class="ic">selected</code> follow its state. Styling alone tells sighted
	...
	<section id="numbers" class="sec">
		...
		<p class="sec__p">
			...
			<code class="ic">Cell</code>s. Numbers are compared by reading down the column, which only works
			when the digits line up: right alignment aligns the units place, and
		...
		<p class="sec__p">
			Format the number yourself — <code class="ic">Intl.NumberFormat</code> with the user's locale.
			Thousands separators and decimal marks are not universal, and a table is exactly where that
			shows.
		</p>
	</section>
	...
	<section id="a11y" class="sec">
		...
		<ul class="sec__p">
			<li>Give it a <code class="ic">Caption</code>. A table announced as just "table" is useless on a page with more than one. It must be the first child of the table. Use <code class="ic">visuallyHidden</code> when a heading above already says it — that hides the picture, not the name.</li>
			<li>Give the identifying cell of each row a <code class="ic">RowHeader</code>. That is what turns "4.2%" into "Bounce rate, Argentina, 4.2%" — without it the value has a column but no subject.</li>
			<li>Put totals in <code class="ic">Footer</code>, not as a last body row. It is not data, and a screen reader user should be told when the rows stop.</li>
			<li>Pass <code class="ic">scrollLabel</code> so the overflow is reachable by keyboard.</li>
			<li>Sortable headers are real <code class="ic">&lt;button&gt;</code>s inside the cell, because a clickable <code class="ic">&lt;th&gt;</code> is neither focusable nor operable by keyboard. The arrow glyph is <code class="ic">aria-hidden</code> — <code class="ic">aria-sort</code> already carries the direction.</li>
		</ul>
	</section>
	...
	<section id="props" class="sec">
		...
		<p class="sec__p" style="margin-top:16px"><code class="ic">Table.Head</code> — a column header.</p>
