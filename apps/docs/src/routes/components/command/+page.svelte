<script lang="ts">
	...
	const rootForwarded: PropRow[] = [
		{ prop: 'label', type: 'string', description: 'Screen-reader-only name for the SEARCH FIELD (renders a hidden label the Input references).' },
		{ prop: 'shouldFilter', type: 'boolean', default: 'true', description: 'Set false when filtering server-side; then render only the items you want shown.' },
		{ prop: 'filter', type: '(value, search, keywords?) => number', description: 'Custom scoring, 0 to 1. Defaults to computeCommandScore.' },
		{ prop: 'onStateChange', type: '(state) => void', description: 'Called when the search, value or filtered set changes.' }
	];

	const itemForwarded: PropRow[] = [
		{ prop: 'value', type: 'string', required: true, description: 'What the query is matched against, and what Root reports as the selected value.' },
		{ prop: 'keywords', type: 'string[]', description: 'Extra terms that should match this item but are not in its label.' },
...
<DocPage group={meta.group} name={meta.name} description={meta.blurb} {toc}>
	<section id="usage" class="sec">
		...
		<p class="sec__p">
			The command-palette pattern. Bits owns the filtering, the scoring and arrow-key navigation, and
			it keeps focus in the Input while the highlight moves — so the user types and navigates without
			ever leaving the field. To open it in a modal, wrap the whole thing in a
	...
	<section id="viewport" class="sec">
		...
		<p class="warn">
			<code class="ic">Command.Viewport</code> goes inside <code class="ic">List</code> and wraps the
			content. It is <strong>not</strong> optional.
		</p>
		<p class="sec__p">
		...
		<p class="sec__p">
			This is not hypothetical: our own a11y suite caught a Command built without one, which is why it
			is called out here rather than left as a detail in the API table.
		</p>
	</section>
	...
	<section id="keywords" class="sec">
		...
		<p class="sec__p">
			...
			heading over nothing. Set <code class="ic">shouldFilter={false}</code> when you filter
			server-side and render only what you want shown.
		</p>
	</section>
