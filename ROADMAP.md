...
- [ ] **Wave 4b (advanced):** Date Picker, Command/Search, Table and Pagination all shipped. **Only Toast remains**, still blocked on three decisions: whether an imperative `import { toast }` is acceptable in a library that is otherwise declarative, whether swipe/stacked animation is in v1, and whether any second runtime dependency is acceptable at all.
...
- [ ] **Custom (non-Bits) components still needed: Toast region only.** Table, Sidebar, Breadcrumb, Sheet, Pagination, Stack and Flex have all shipped; Textarea and Skeleton shipped in Wave 4a. Carousel (embla) was never scoped and is not in the registry — decide whether it belongs before adding it back.
