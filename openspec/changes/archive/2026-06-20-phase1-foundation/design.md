...
Chakra-style raw colors in `src/lib/theme/*` are kept as a _starting palette_ but
...
CSS, so the only thing that can flash is the _class_, and that is the smallest possible
...
helper emits _class names + data attributes_, not inline styles. **Rationale**: keeps style
