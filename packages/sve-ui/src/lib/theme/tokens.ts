...
export function themeToVars(theme: PartialSveTheme): string {
	...
	if (theme.typography) {
		...
		if (typo.family)
			vars += flattenToVars(typo.family as unknown as NestedRecord, '--sve-font-family');
		if (typo.size) vars += flattenToVars(typo.size as unknown as NestedRecord, '--sve-font-size');
		if (typo.weight)
			vars += flattenToVars(typo.weight as unknown as NestedRecord, '--sve-font-weight');
		if (typo.lineHeight)
			vars += flattenToVars(typo.lineHeight as unknown as NestedRecord, '--sve-line-height');
	}
