<script lang="ts">
	...
	let { size = 'md', shape = 'circle', class: cls, children, ...rest }: Props = $props();

	const className = $derived(
		['sve-avatar', `sve-avatar--${size}`, `sve-avatar--${shape}`, cls ?? '']
			.filter(Boolean)
			.join(' ')
	);
</script>

<Avatar.Root class={className} data-slot="avatar-root" {children} {...rest} />
...
<style>
	...
	/* --- Sizes --- */
	:global(.sve-avatar--sm) {
		width: 2rem;
		height: 2rem;
		font-size: var(--sve-font-size-sm);
	}
	:global(.sve-avatar--md) {
		width: 2.5rem;
		height: 2.5rem;
		font-size: var(--sve-font-size-md);
	}
	:global(.sve-avatar--lg) {
		width: 3.5rem;
		height: 3.5rem;
	...
	/* --- Shapes --- */
	:global(.sve-avatar--circle) {
		border-radius: var(--sve-radius-full);
	}
	:global(.sve-avatar--square) {
