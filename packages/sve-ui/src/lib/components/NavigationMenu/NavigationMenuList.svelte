...
<NavigationMenu.List
	class={['sve-navigation-menu__list', cls].filter(Boolean).join(' ')}
...
<style>
	:global(.sve-navigation-menu__list) {
		display: flex;
		align-items: center;
		gap: var(--sve-space-1);
		margin: 0;
	...
	:global(.sve-navigation-menu[data-orientation='vertical'] .sve-navigation-menu__list) {
