...
<Dialog.Portal>
  <SheetOverlay />
  <Dialog.Content
    class={['sve-sheet', `sve-sheet--${side}`, `sve-sheet--${size}`, cls]
      .filter(Boolean)
      .join(' ')}
...
<style>
  ...
  :global(.sve-sheet--right.sve-sheet--sm) { width: min(90vw, 18rem); }
  :global(.sve-sheet--left.sve-sheet--md),
  :global(.sve-sheet--right.sve-sheet--md) { width: min(90vw, 24rem); }
  :global(.sve-sheet--left.sve-sheet--lg),
  :global(.sve-sheet--right.sve-sheet--lg) { width: min(90vw, 32rem); }

  :global(.sve-sheet--top.sve-sheet--sm),
  :global(.sve-sheet--bottom.sve-sheet--sm) { height: min(85vh, 12rem); }
  :global(.sve-sheet--top.sve-sheet--md),
  :global(.sve-sheet--bottom.sve-sheet--md) { height: min(85vh, 18rem); }
  :global(.sve-sheet--top.sve-sheet--lg),
  :global(.sve-sheet--bottom.sve-sheet--lg) { height: min(85vh, 26rem); }
</style>
