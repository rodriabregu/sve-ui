...
describe('Checkbox', () => {
	...
	it('applies md size by default and sm when requested', () => {
		...
		expect(a.querySelector('[data-slot="checkbox"]')?.classList.contains('sve-checkbox--md')).toBe(
			true
		);
		const { container: b } = render(CheckboxFixture, { props: { size: 'sm' as const } });
		expect(b.querySelector('[data-slot="checkbox"]')?.classList.contains('sve-checkbox--sm')).toBe(
			true
		);
	});
