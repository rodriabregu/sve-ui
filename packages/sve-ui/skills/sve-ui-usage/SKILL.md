...
- Wrap any region that loads asynchronously in `Busy`: `<Busy busy={loading} label="Loading projects" doneLabel={`${n} projects loaded`}>`. Nothing else in the library can say a region is loading — `Spinner` is decorative and announces nothing. ALWAYS pass `doneLabel` saying what arrived: telling the user it is loading and never that it finished is worse than silence. It waits 400ms before announcing, so a fast response is not narrated.
...
- `Field`'s `required` always marks the label, but only SOME controls can announce it: native inputs use the real attribute, Bits sets `aria-required` on checkbox/switch/radiogroup/rating-group, and `Slider` puts it on the thumb. `Select`, `Toggle` and `DatePicker` render `<button>`s and CANNOT be announced as required — axe rejects `aria-required` there — so they swallow it. Never rely on `required` alone; validate and say what is wrong.
- On a failed submit call `await focusFirstInvalidField({ root: formEl })` (exported from `sve-ui`). Do NOT make the error a live region: one that is both live and referenced by `aria-describedby` is announced twice. Focusing reads label + error + description in one go. It awaits `tick()` itself and returns `false` when nothing was focused.
- Every form control takes `invalid`. Prefer driving it from `Field`: passing `Field` an `error` is what makes a field invalid, so the message and the control cannot disagree. `aria-invalid` is set only where the rendered role supports it (checkbox, switch, radiogroup, combobox, slider) — on `Select.Trigger` and `Toggle`, which render plain `<button>`s, the accessible signal is `Field`'s `aria-describedby`.
- `Button` takes `loading`: spinner + `aria-busy`, activation blocked, and it stays FOCUSABLE on purpose (`disabled` would drop the keyboard user's focus to the top of the document). Pass `loadingLabel` saying what is happening, not that something is.
